...
import cv2
from utils import softmax, sigmoid, _draw_rotate_rec,\
    cal_angle, get_corner_gtbox, read_anchors_file, \
    read_labels, non_max_supression, filter_bbox
    ...
    for i in range(grid_h):
        for j in range(grid_w):
            for k in range(n_anchors):
                ...
                h = np.exp(data[0, i, j, k, 3]) * anchors[k][1] / 60 * grid_h * net_scale#label error
...
def predict(draw_gtbox = False):
    ...
    saver = tf.train.import_meta_graph(
        './models_serve/yolo_tloss_0.1686321645975113_vloss_3.5061778074287506-90000.meta')
    saver.restore(sess, './models_serve/yolo_tloss_0.1686321645975113_vloss_3.5061778074287506-90000')
    ...
    for img_idx, rgb_map, target in dataset.getitem():
        ...
        if draw_gtbox:
            for i in range(target.shape[0]):
                ...
                rz = target[i][5]
                _draw_rotate_rec(img, cx, cy, w, h, rz, gt_color)
                label = class_list[int(target[i][0])]
                box = get_corner_gtbox([cx, cy, w, h])
                cv2.putText(img, label, (box[0], box[1]), cv2.FONT_HERSHEY_PLAIN, 1.0, gt_color, 1)
        ...
        for box in all_bboxs:
            ...
            class_prob = box[7]
            _draw_rotate_rec(img, box[1], box[2], box[3], box[4], angle, colors[class_idx])
