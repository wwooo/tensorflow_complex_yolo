...
calib_dir = '../../kitti/KITTI/training/calib/'
...
def rz_to_ry(rz):
    ...
    return angle
def  image_to_velo(hy, wx):
    velo_x = hy * 60 / 768.0
    velo_y = (wx - 512) * 40 / 512.0
    return velo_x, velo_y
def velo_to_cam(velo_x, velo_y, tr):
    ...
    for test_file_index in range(1000):
        ...
        with open(result_file, "w") as f:
            for box in all_bboxs:
                pred_img_y = box[2]
                pred_img_x = box[1]
                velo_x, velo_y = image_to_velo(pred_img_y, pred_img_x)
                cam_x, cam_z = velo_to_cam(velo_x, velo_y, calib['Tr_velo2cam'])
